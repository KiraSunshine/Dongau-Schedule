# Спецификация API «Свободные аудитории» ДонГУ

Документация и техническое описание API свободных аудиторий корпусов ДонГУ.

> [!NOTE]
> Для ДонГУ сервис свободных аудиторий **пока не подключён**: публичный API университета не предоставляет данные о занятости аудиторий. Раздел планируется. Приведённая ниже схема данных — целевая, и будет использоваться после подключения источника данных.

---

## 1. Общие сведения и архитектура

API предоставляет актуальную матрицу занятости и доступности учебных аудиторий по корпусам университета на 14 дней вперёд.

- **Формат данных**: JSON (UTF-8, gzip-совместимый)
- **Точка доступа (CDN)**:
  ```http
  GET https://raw.githubusercontent.com/KiraSunshine/Dongau-Schedule/gh-pages/free_rooms.json
  ```
  *(Зеркало GitHub Pages: `https://kiraSunshine.github.io/Dongau-Schedule/free_rooms.json`)*
...
| `campuses` | `Array<String>` | Доступные идентификаторы корпусов университета |
...
| `rooms[].name` | `String` | Номер/название аудитории |
| `rooms[].fullTitle` | `String` | Полное отображаемое наименование с корпусом |
| `rooms[].campus` | `String` | Корпус расположения аудитории |
