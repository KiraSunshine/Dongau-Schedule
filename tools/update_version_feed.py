...
BUNDLE_ID = "ru.dongau.schedule"
TINT_COLOR = "4F46E5"
APP_DESCRIPTION = (
    "Расписание пар ДонГУ: поиск свободных аудиторий, "
    "задачи и офлайн-кеш."
)
...
def parse_app_version(path):
    ...
    repo = m.group(1) if m else "KiraSunshine/Dongau-Schedule"
...
def asset_urls(repo, channel, version):
    if channel == "stable":
        ...
        return {
            ...
            "apk_url": f"{base}/latest/download/Schedule-Dongau.apk",
            "ipa_url": f"{base}/latest/download/Schedule-Dongau.ipa",
        }
    if channel in ("beta", "rc"):
        base = f"https://github.com/{repo}/releases/download/v{version}"
        return {
            "download_url": base,
            "apk_url": f"{base}/Schedule-Dongau-v{version}.apk",
            "ipa_url": f"{base}/Schedule-Dongau-v{version}.ipa",
        }
    # preview (rolling dev)
    base = f"https://github.com/{repo}/releases/download/preview"
    return {
        "download_url": f"{base}/Dongau-Schedule-preview.apk",
        "apk_url": f"{base}/Dongau-Schedule-preview.apk",
        "ipa_url": f"{base}/Dongau-Schedule-preview.ipa",
    }
...
def build_source(repo, channel, version, ipa_url):
    ...
    return {
        "name": "ДонГУ Расписание" + ("" if stable else " (Beta)"),
        "identifier": f"dongau-schedule-{'stable' if stable else 'beta'}",
        "sourceURL": f"https://raw.githubusercontent.com/{repo}/gh-pages/{filename}",
        "apps": [{
            "name": "Расписание ДонГУ",
            "bundleIdentifier": BUNDLE_ID,
            "developerName": "KiraSunshine",
