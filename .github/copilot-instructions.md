# GitHub Copilot Instructions for Dongau-Schedule

## О проекте
**Dongau-Schedule** (ДонГУ Расписание) — кроссплатформенное мобильное приложение расписания для студентов и преподавателей ДонГУ (Донецкий Государственный Университет) на базе **Kotlin Multiplatform (KMP)** и **Compose Multiplatform** для Android и iOS.
...
- `shared/src/commonMain/kotlin/com/jetbrains/kmpapp/`:
  - `data/`: Репозитории (`ScheduleRepository`, `TaskRepository`, `FreeRoomsRepository`), API (`DongauScheduleApi`), парсер iCal (`DongauScheduleParser`), модели данных (`Lesson`, `StudyTask`, `Subject`, `ScheduleTarget`), система кэширования (`UnifiedSyncManager`).
  - `screens/`:
    ...
    - `rooms/`: Поиск свободных аудиторий по корпусам и этажам с интерактивным выбором даты.
