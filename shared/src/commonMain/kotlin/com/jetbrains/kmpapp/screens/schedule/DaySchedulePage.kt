...
internal fun DaySchedulePage(
    ...
    if (slots.isEmpty() || slots.all { it is ScheduleSlot.Empty && errorMessage != null }) {
        Box(modifier = modifier, contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                if (errorMessage != null) {
                    Text("⚠️", fontSize = 48.sp)
                    Spacer(modifier = Modifier.height(12.dp))
                    Text("Не удалось загрузить расписание", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center)
